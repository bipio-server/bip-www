...
define(function(require) {
    ...
    var Scheduler = function (element, options) {
        ...
        this.$timeZone = $('.scheduler-timezone select');
...
*/
        this.$element.find('.radio').radio();

        // generates a signed integer from valueStr and offset
        function offsetVal(valuStr, offset) {
            var sign = 1;
            if (-1 !== valuStr.indexOf('-')) {
                sign *= -1;
            }

            return Number(offset) * sign;
        }
        ...
        var opts = {
            ...
            sortResults : function(results, container, query) {
                results = results.sort(function(a, b) {
                    ...
                    // look for time offsets
                    if (lTokens.length > 1 && rTokens.length > 1 && lTokens[0] == rTokens[0]) {

                        lOffs = offsetVal(a, lTokens[1]);
                        rOffs = offsetVal(b, rTokens[1]);
    ...
    Scheduler.prototype = {
        ...
        getValue: function(){
            ...
            if (startDate) {
                ...
                startDateTime = '' + getFormattedDate(startDate, '-');
    ...
    //            startDateTime += (timeZone.offset==='+00:00') ? 'Z' : timeZone.offset;
                startDateTime += moment().tz(timeZone).format('Z');
            }
        ...
        repeatIntervalSelectChanged: function(e, data, spinValue) {
            var selectedItem, val, txt, spinValue;
            ...
            switch(val.toLowerCase()) {
                ...
                case 'weekly':
                case 'monthly':
//                    this.$repeatIntervalSpinner.spinner('max', 0);
                    this.$repeatIntervalSpinner.spinner('value', spinValue || 1);
                    this.$repeatIntervalSpinner.spinner('setOptions', { step : 1, min: 1, max : 999 });
                    this.$repeatIntervalPanel.show();
                    break;
                case 'minutely':
//                    this.$repeatIntervalSpinner.spinner('step', 15).spinner('max', 30);
//                    this.$repeatIntervalSpinner.spinner('max', 30);

//                    this.$repeatIntervalSpinner.spinner('value', spinValue || 15);
//                    this.$repeatIntervalSpinner.spinner('setOptions', { min: 15, step : 15, max : 30 });

                    this.$repeatIntervalSpinner.spinner('value', spinValue || 5);
                    this.$repeatIntervalSpinner.spinner('setOptions', { min: 5, step : 5, max : 30 });
        ...
        setValue: function(options){
            ...
            this.$timeZone.select2("val", userSettings.timezone);
            ...
            if(options.timeZone){
                ...
                */
                this.$timeZone.select2("val", options.timeZone.name );
            ...
            if(options.recurrencePattern){
                ...
                if(recur.FREQ==='DAILY'){
                    ...
                    item = 'minutely';
                    this.$repeatIntervalSpinner.spinner('value', 15);
                    this.$repeatIntervalSpinner.spinner('setOptions', { min: 15, step : 15, max : 30 });
                ...
                var spinValue;
                if(recur.INTERVAL){
                    spinValue = parseInt(recur.INTERVAL, 10)
...
//                    this.$repeatIntervalSpinner.spinner('value', parseInt(recur.INTERVAL, 10));
                ...
                this.repeatIntervalSelectChanged(null, null, spinValue);
            }
        },
